...
그러다 최근 업무에서 `EKS`(Amazon Elastic Kubernetes Service)와 `Helm`을 다루게 되면서, 잊고 있던 기본기를 다시 확실히 익혀야겠다는 생각이 들었다. 그래서 **《미니멀리즘 프로그래머》**와 같은 가이드들을 참고하며 복습한 내용들을 실습과 함께 정리해 보려 한다.
