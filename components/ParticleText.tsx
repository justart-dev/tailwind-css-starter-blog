...
const resolveColorVar = (value: string, container: HTMLElement): string => {
...
const waitForFonts = async (font: string): Promise<void> => {
  ...
  try {
    ...
    // Font may already be loaded or unsupported; continue
  }
...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async (): Promise<void> => {
      ...
      resolvedColor = resolveColorVar(color, container)
      resolvedHighlight = resolveColorVar(highlightColor, container)
      ...
      // Measure the widest line for font scaling
      offCtx.font = font
      let maxWidth = 0
      ...
      if (maxWidth > maxTextWidth) {
        resolvedSize = Math.max(18, resolvedSize * (maxTextWidth / maxWidth))
      ...
      // Use first line metrics for layout
      const metrics = offCtx.measureText(txtLines[0])
      const left = Math.ceil(metrics.actualBoundingBoxLeft || 0)
      const right = Math.ceil(metrics.actualBoundingBoxRight || metrics.width)
      const ascent = Math.ceil(metrics.actualBoundingBoxAscent || resolvedSize * 0.78)
      const descent = Math.ceil(metrics.actualBoundingBoxDescent || resolvedSize * 0.22)
      const lineGap = Math.ceil(resolvedSize * 0.12)
      const padding = Math.max(12, Math.ceil(resolvedSize * 0.08))
      const textWidth = Math.max(1, left + right)
      const textHeight = Math.max(
        1,
        txtLines.length * (ascent + descent) + (txtLines.length - 1) * lineGap
      )
      ...
      txtLines.forEach((line, i) => {
        const y = padding + ascent + i * (ascent + descent + lineGap)
        offCtx.fillText(line, padding - left, y)
      })
      ...
      particles = selected.map((target, index) => {
        ...
        const blend =
          baseRgb && highlightRgb
            ...
            : 0
        const particleColor =
          baseRgb && highlightRgb ? rgbToCss(mixRgb(baseRgb, highlightRgb, blend)) : resolvedColor
